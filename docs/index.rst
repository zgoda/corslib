...
* strictly no origin reflection - ``Allow-Origin`` header value is taken only from rule definition and never reflected from request ``Origin`` value unless policy resolves with exact match from non-exact rule (``PATH`` or ``REGEX``);
...
Library includes extensive range of well tested rule validators based on `OWASP WSTG <https://owasp.org/www-project-web-security-testing-guide/>`_ to prevent common security lapses coming from CORS misconfiguration.
